...
impl Server {
    ...
    async fn handle_execution_requests(
        ...
        loop {
            ...
            *self.latest_execution_request.lock().await = Some(message.clone());
            ...
            let eval_result = context.lock().await.execute(src);
            match eval_result {
                Ok(output) => {
                    if !output.is_empty() {
    ...
    async fn handle_shell_message<S: zeromq::SocketRecv + zeromq::SocketSend>(
        ...
        if message.message_type() == "kernel_info_request" {
            ...
            // Jupyter-erg doesn't support this yet.
            ...
            || message.message_type() == "comm_info_request"
        {
            ...
            // Jupyter-erg doesn't handle this yet.
...
/*async fn comm_open(
    ...
    if message.target_name() == "jupyter-erg-cargo-check" {
...
fn kernel_info() -> JsonValue {
    object! {
        ...
        "banner" => format!("Jupyter-erg {}", env!("CARGO_PKG_VERSION")),
        "help_links" => array![
            object!{"text" => "Erg std docs",
                    "url" => "https://github.com/erg-lang/erg/tree/main/doc/EN/API"}
        ],
        "status" => "ok"
    }
}
